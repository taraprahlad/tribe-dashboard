...
def summarize_by_column(df: pd.DataFrame, column: str) -> dict[str, int]:
    ...
    # To-do: convert the pandas Series 'counts' into a plain dict.
...
def audio_duration_stats(df: pd.DataFrame) -> dict[str, float] | None:
    ...
    # to-do: build and return a dict with keys "mean", "median", "min", "max".
    durationdict = {
        "mean" : durations.mean(),
...
def missing_value_report(df: pd.DataFrame) -> dict[str, int]:
    ...
    # to-do: build {column_name: number_of_missing_values} for every column
