...
i played around with saliency maps for the stg and occipital pole reacting to a 30s clipping of the reading of the picture of dorian gray ch.13. there was a correlation of almost 0.88 percent! it seems like raw saliency is dominated by input magnitude but once you take z-scores to scale it, because the maps are normalized now, it seems like the activations across the regions aren't as different. it seems like vanilla saliency can tell you every place where the output is locally sensitive (because it's dominated by a shared magnitde signal) while integrated gradients(IG) saliency can tell you which inputs actually moved the prediction away from its clip-typical value. for STG at t = 30 for the dorian audio clip, that's almost a 15-frame window. there's a peak at t=57 for IG specifically and that's suspicious -- i think it's strange for a spike to be sitting close to my attributed timestep so i investigated it some more. the peaks for both the vanilla and IG matched at index 58 and it was when the narrator was reading the librivox boilerplate -- specifically at index 58, or roughly 17 seconds, he said "librivox." it made me realize that auto-clipping librivox audio to 60s includes the spoken attribution boilerplate creates a high-salience speech onset that ends up dominating STG attribution -- it means that any per-frame attribution is skewed near the front. the fix going forward is probably to trim the boilerplate before inference, or at least ignore the opening frames when reading attribution curves. the actual speech onset driver is second 33 of the clip.

**fuckass colab upgraded and i lost my patience figuring out what to do**
my old torch pins for running the model in google colab no longer worked because tribe switched to torch 2.11 / python 3.12 -- i had to mess around with all these different versions before i finally realized that torch + torchaudio need to be from the same build/index. i also ended up having to use a cpu in order to avoid cuda driver-wheel matching.

**method notes**
ablation is apparently forward-only -- i didn't have to uses requires_grad the way i did in ig. i pooled over timestep so that it would be easy to compare different regions, however the pooled STG scalar specifically was SIGNED so i had to interpret what the signs meant on the graph.